...
**Главная страница: меню на казахском, последний статус, загрузка анализа**

![upload](docs/screenshots/01_upload.png)

Остальные экраны (подтверждение значений, объяснение KZ/RU, история, чат) будут показаны на защите. Скриншоты с реальными значениями не публикуются.
